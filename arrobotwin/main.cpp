...
int main(int argc, char *argv[])
{
	...
	ipclis = (HANDLE)_beginthreadex(NULL,       // security  
		...
		&uiThread1ID);
	if (ipclis == 0)
		cout << "Failed to create ipclisten thread " << endl;
...
static unsigned __stdcall robotControlThread(void *)
{
	...
	while (1){
		...
		else if (g_robotControl.dist<80 && fabs(g_robotControl.theta)>20)arrobot.setVel2(g_robotControl.theta, -g_robotControl.theta);
		else if (g_robotControl.dist > 80)arrobot.setVel2(0.4*g_robotControl.dist + g_robotControl.theta, 0.4*g_robotControl.dist - g_robotControl.theta);
		else {
			arrobot.setVel2(0, 0);
